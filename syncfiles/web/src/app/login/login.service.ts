...
export class LoginService {
    constructor(private http:Http,private accountEventService:AccountEventsService,private router: Router, private config : ConfigService) {
    }
    authenticate(username:string,password:string):Observable<Account> {
        ...
        return this.http.post(this.config.urls.BACKEND_API_ROOT_URL+AppUtils.BACKEND_API_AUTHENTICATE_PATH, JSON.stringify({login:username,password:password}),{headers:headers})
            .map((res:Response) => {
    ...
    logout(callServer:boolean = true):void {
        ...
        if(callServer) {
            this.http.get(this.config.urls.BACKEND_API_ROOT_URL + '/logout').subscribe(() => {
